...
import { lookupPerk } from "../services/manifest-cache.ts";
import { buildCompareRows } from "../services/compare.ts";
import { DestinyComponentType } from "../utils/constants.ts";
import { dim, error, header } from "../ui/format.ts";
import { loadInventoryContext, locationLabel, runCommandAction } from "./shared.ts";
...
export function registerCompareCommand(program: Command) {
  program
    ...
    .action(runCommandAction(async (query: string, opts) => {
      ...
      const rows = buildCompareRows(
        matches,
        (location) => locationLabel(location, byCharacterId),
        lookupPerk
      );
