...
describe("equip command", () => {
  ...
  test("transfers a vault item then equips it using shared inventory context", async () => {
    ...
    const equipItemMock = mock(async () => undefined);
    const spinner = {
      text: "",
      start: mock(function () {
        return spinner;
    ...
    const { registerEquipCommand } = await import(
      `./equip.ts?t=${Date.now()}-${Math.random()}`
    );

    const result = await runCommand(registerEquipCommand, [
      "equip",
      "Ace",
      "--character",
      "hunter",
    ...
    expect(equipItemMock).toHaveBeenCalledWith("inst-vault-1", "hunter-id");
  });
